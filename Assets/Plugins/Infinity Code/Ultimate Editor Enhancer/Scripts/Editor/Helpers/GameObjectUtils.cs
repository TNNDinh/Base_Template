...
namespace InfinityCode.UltimateEditorEnhancer
{
    public static class GameObjectUtils
    {
        ...
        {
            if (UnityEditorInternal.InternalEditorUtility.tags.All(t => t != "Collection")) return;

            GameObject[] groups = GameObject.FindGameObjectsWithTag("Collection");
            if (groups.Length == 0) return;

            Transform firstParent = targets[0].transform.parent;
            string activeGroup = (firstParent && firstParent.tag == "Collection")? firstParent.gameObject.name: null;
        ...
        public static bool IsInUntitledScene(GameObject gameObject)
        {
            return !EditorUtility.IsPersistent(gameObject) && string.IsNullOrEmpty(gameObject.scene.path);
        }
            ...
            if (!isMultiple)
            {
                ...
                if (!string.IsNullOrEmpty(assetPath))
                {
                    ...
                    menu.Add("Select Prefab Asset", () =>
                    {
                        ...
                        EditorCompatibility.PingObject(Compatibility.GetInstanceID(asset));
                        EditorMenu.Close();
                    });
                }
            }
