...
namespace InfinityCode.UltimateEditorEnhancer.SceneTools
{
    ...
    public static class SelectionSize
    {
        ...
        {
            if (!IsSelectionBoundsHotkeyPressed() && !forcedShow) return;
        ...
        private static bool IsSelectionBoundsHotkeyPressed()
        {
            if (Prefs.selectionBoundsKeyCode == KeyCode.CapsLock) return Event.current.capsLock;

            EventModifiers modifiers = Event.current.modifiers;
            if ((modifiers & Prefs.selectionBoundsModifiers) != Prefs.selectionBoundsModifiers) return false;
            if ((modifiers & ~Prefs.selectionBoundsModifiers) != EventModifiers.None) return false;

            return KeyManager.IsKeyDown(Prefs.selectionBoundsKeyCode);
        }
