...
namespace InfinityCode.UltimateEditorEnhancer.Windows
{
    public partial class Search
    {
        ...
        {
            tempRecords.Add(Compatibility.GetInstanceID(go), new GameObjectRecord(go));
            Component[] components = go.GetComponents<Component>();
            for (int i = 0; i < components.Length; i++)
            {
                Component c = components[i];
                tempRecords.Add(Compatibility.GetInstanceID(c), new ComponentRecord(c));
            }
        ...
        private static void CacheSceneGameObjects()
        {
            ...
            for (int i = 0; i < transforms.Length; i++)
            {
                ...
                int key = Compatibility.GetInstanceID(go);
        ...
        private static void CacheSceneItems()
        {
            ...
            {
                Component c = components[i];
                int key = Compatibility.GetInstanceID(c);
                Record r = null;
                ...
                key = Compatibility.GetInstanceID(go);
