...
namespace InfinityCode.UltimateEditorEnhancer.InspectorTools
{
    public class EmptyInspector: InspectorInjector
    {
        ...
        private static void InitSearch(VisualElement parent)
        {
            IMGUIContainer search = new IMGUIContainer(instance.DrawFilterTextField)
            {
                name = SearchElementName,
                style =
                {
                    backgroundColor = Styles.isProSkin ? new Color(.22f, .22f, .22f) : new Color(.76f, .76f, .76f),
                    marginLeft = 3,
                    marginRight = 5,
                    paddingTop = 5,
                    paddingBottom = 5,
        ...
        protected override bool OnInject(EditorWindow wnd, VisualElement mainContainer, VisualElement editorsList)
        {
            ...
            {
                if (mainContainer[i].name == SearchElementName)
                {
            ...
            if (searchElement == null) InitSearch(mainContainer);

            editorsList.parent.Insert(0, visualElement);
            mainContainer.Insert(mainContainer.childCount > 0 && mainContainer[0].name == "InspectorBar" ? 1 : 0, searchElement);
            filterText = "";
            UpdateFilteredItems();
        ...
        public static void ResetCachedItems()
        {
            ...
            if (searchElement != null)
            {
                if (instance != null)
                {
                    searchElement.RemoveFromHierarchy();
                    searchElement = null;
                }
                else searchElement = null;
            }
        }
