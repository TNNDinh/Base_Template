...
public class InfiniteScroll : MonoBehaviour, IBeginDragHandler, IDragHandler, IScrollHandler
{
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        else if (scrollContent.Horizontal) positiveDrag = eventData.position.x > lastDragPosition.x;
    ...
    {
        var currItemIndex = positiveDrag ? scrollRect.content.childCount - 1 : 0;
        var currItem = scrollRect.content.GetChild(currItemIndex);

        if (!ReachedThreshold(currItem)) return;

        var endItemIndex = positiveDrag ? 0 : scrollRect.content.childCount - 1;
    ...
    private bool ReachedThreshold(Transform item)
    {
        if (scrollContent.Vertical)
        {
            var posYThreshold = transform.position.y + scrollContent.Height * 0.5f + outOfBoundsThreshold;
            var negYThreshold = transform.position.y - scrollContent.Height * 0.5f - outOfBoundsThreshold;
            return positiveDrag
                ? item.position.y - scrollContent.ChildWidth * 0.5f > posYThreshold
                : item.position.y + scrollContent.ChildWidth * 0.5f < negYThreshold;
        }

        var posXThreshold = transform.position.x + scrollContent.Width * 0.5f + outOfBoundsThreshold;
        var negXThreshold = transform.position.x - scrollContent.Width * 0.5f - outOfBoundsThreshold;

        Debug.Log(
            $"item.position.x {item.position.x - scrollContent.ChildWidth * 0.5f}, posXThreshold {posXThreshold}");
        return positiveDrag
